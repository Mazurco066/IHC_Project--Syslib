...
type
  Tfrm_categorias = class(TForm)
    ...
    procedure btn_atClick(Sender: TObject);
  private
...
uses Form_Dados, Form_Livro_Incluir;
...
procedure Tfrm_categorias.btn_atClick(Sender: TObject);
begin

  if (ValidaCampos = false) or (lbl_id.Caption = '?') then begin
    exit;
  end else begin
    Form_Dados.Modulo_dados.AtualizarCategoria(strtoint(lbl_id.Caption),txt_cat.Text);
