...
begin
  ...
  if(ValidaCampos = false) or (lb_id.Caption = '?') then begin
    exit;
  end else begin
...
procedure Tfrm_aluno.Button3Click(Sender: TObject);
begin

  if(lb_id.Caption = '?') then begin
    showmessage('Especifique a qual aluno excluir!');
    exit;
  end else begin
    Form_Dados.Modulo_Dados.ExcluirAluno(strtoint(lb_id.Caption));
